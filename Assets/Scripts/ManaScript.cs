...
public class ManaScript : ClickableObject {
	...
	public void Reset(){
		// Return to original colour, turn off particles
		selectFX.Stop();
		value = savedValue;
    ...
    private void UseOption(GameObject wedgeHalf) {
		// Copy value and material of selected option, while saving the originals
		...
		// Get the black mana and add it to player's hand
		for(int i = 1; i < wedgeHalf.transform.childCount; i++){
